...
/* The client owns RDMA; callbacks own HTTP control. READY overlaps the data
 * transfer. PUT writes to the staging MR advertised by PREPARE.
 *
 * Core state is guarded by apiLock; callbacks own their
...
struct V2State {
  ...
  /* Bumped by every init and shutdown; checked as an internal transfer
   * precondition under apiLock. */
...
void fillCommonRequest(hipObjTransferReqV2_t& req, const char* method,
                       ...
                       uint64_t transferBudgetMs, uint64_t remainingMs) {
  ...
  /* deadlineMs reports the whole-transfer budget remaining per the
   * header contract; remainingMs reports the budget relevant to this
   * callback, which on the post-expiry cancel path is the fresh
   * cleanup allowance rather than the (spent) transfer budget. */
  req.deadlineMs = static_cast<uint32_t>(
    transferBudgetMs > 0xffffffff ? 0xffffffff : transferBudgetMs);
...
/* Creates the per-transfer QP/CQ pair with a retired-ring reservation
 * and pins the buffer MR. Returns 0 on success, -1 on failure, or -2
 * when rollback fails. On failure, release acquired references or retain
 * surviving verbs objects for shutdown. */
int acquireSession(void* devPtr, SessionResources& res,
  ...
  const int cret = createRcConnV2(v2State().device, res.conn, &rollbackFailed);
  if (cret != 0) {
    if (!rollbackFailed) {
    ...
    if (cqParkThrew || res.id == 0) {
      /* Park survivors for a later shutdown retry. */
      bool qpIgnored = false;
      bool cqOk = false;
      destroyRcConnV2(res.conn, &qpIgnored, &cqOk);
      bool settled = cqOk ||
                     parkEmergency(emergencyHandle, res.conn, retireRid);
      g_bufferMap.releaseMrRef(devPtr);
      if (cqOk) {
        reg.retired().unreserve(retireRid);
      }
  ...
  if (insertThrew || res.id == 0) {
    ...
    if (!qpOk || !cqOk) {
      /* Keep the retirement reservation with any surviving QP; release the
       * device reference. */
...
int releaseSession(SessionResources& res) {
  if (res.pinnedBuffer != nullptr) {
    ...
    const int rc = releaseConnection(res.id);
...
hipObjError_t finalizeOutcome(hipObjError_t wireOutcome, int releaseRc) {
  ...
  /* Preserve the deadline diagnostic when release failure overrides the
   * status. */
  return {hipObjInternalError, wireOutcome.hipError == kDiagDeadlineExpired
                                 ? kDiagDeadlineExpired
                                 : 0};
}

bool v2IsInitialized() {
  return v2State().initialized;
}
...
const char* v2NicName() {
  return v2State().nicName.c_str();
}

int v2SelectedPort() {
  return v2State().selectedPort;
}

int v2SelectedGidIndex() {
  return v2State().selectedGidIndex;
}

uint64_t v2InitGeneration() {
  return v2State().initGeneration;
}
...
int v2Init(hipObjConfigV2_t* config) {
  ...
  int ret = -1;
  try {
    /* Convert device-open failures to hipObjRdmaError. */
    ret = (devName != nullptr) ? hipObj::openRdmaDeviceByName(devName, raw)
...
int v2Transfer(int isPut, const char* bucket, const char* key, void* devPtr,
               ...
               bool haveEntryMs, uint64_t snapshotGeneration, bool haveSnapshot,
               int* diagOut) {
  V2State& st = v2State();
  if (!st.initialized) {
  ...
  assert(!haveSnapshot || snapshotGeneration == st.initGeneration);
  ...
  const auto cleanup = [&](hipObjError_t wireOutcome) -> int {
    ...
    if (wireCancelEligible && !sessionId.empty() && !cancelSent &&
      ...
      fillCommonRequest(creq, isPut ? "PUT" : "GET", bucket, key, size, offset,
                        query, 0, 0, deadline, remaining(deadline),
                        cancelBudgetMs);
  ...
  try {
    do {
      ...
      /* Require an RC token with a nonzero GID. */
      ...
      const bool sessionHex = [&] {
        ...
        for (size_t i = 0; i < 32; ++i) {
          if (!isxdigit(static_cast<unsigned char>(q[i])))
            return false;
        }
        return q[32] == '\0';
      }();
      ...
      const uint64_t connectBudgetMs = st.connectDeadlineMs != 0
                                         ? st.connectDeadlineMs
                                         : kDefaultConnectDeadlineMs;
      const uint64_t nowMs0 = steadyNowMs();
      const uint64_t connectDeadline = nowMs0 + connectBudgetMs < deadline
                                         ? nowMs0 + connectBudgetMs
                                         : deadline;
      ...
      /* Post the receive before READY to avoid RNR retries from a fast
       * peer. */
      ...
      if (!isPut) {
        ...
        dataOk = pollDeadline(res.conn.cq, IBV_WC_RECV_RDMA_WITH_IMM, deadline,
                              wc) &&
      ...
      if (!finalOk) {
        outcome = finalExpired
                    ? hipObjError_t{hipObjS3Error, kDiagDeadlineExpired}
                    : hipObjError_t{hipObjS3Error, 0};
        break;
      }
