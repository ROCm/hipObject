...
public:
  ...
  struct ibv_mr* reg_mr_host(struct ibv_pd* pd, void* addr,
                             size_t length, int access);
...
private:
  ...
  struct IbvFuncs {
    ...
    struct ibv_mr* (*reg_dmabuf_mr)(struct ibv_pd*, uint64_t, size_t,
                                    uint64_t, int, int);
    struct ibv_mr* (*reg_mr_iova2)(struct ibv_pd*, void*, size_t,
                                   uintptr_t, int);
