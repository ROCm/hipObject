...
# With a real ernic device we expect a full round-trip (exit 0).
# The token line proves the RC QP came up.
echo "${put_out}" | grep -q "would send RDMA token\|PUT ok" || {
    echo "ERROR: PUT did not reach RDMA token stage"
    exit 1
}
...
echo "${get_out}" | grep -q "would send RDMA token\|GET ok\|Data integrity" || {
    echo "ERROR: GET did not reach RDMA token stage"
    exit 1
}
